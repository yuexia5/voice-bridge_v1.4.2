...
type AstrBotState = "idle" | "connecting" | "connected" | "ready" | "closed";
type GeminiApiFormat = "google" | "openai";
type VoiceReplyMode = "astrbot" | "backend_model";
type AsrProvider = "openai_chat_audio" | "openai_transcriptions";
...
interface RuntimeConfig {
  ...
  mimoTtsModel: string;
  mimoTtsVoice: string;
  mimoTtsFormat: "wav" | "mp3";
  mimoTtsStyle: string;
  geminiApiKey: string;
  geminiBaseUrl: string;
  geminiModel: string;
  geminiApiFormat: GeminiApiFormat;
  geminiOpenAiResponseFormat: "none" | "json_object";
...
const config: RuntimeConfig = {
  ...
  geminiBaseUrl: normalizeBaseUrl(
    readEnv("GEMINI_BASE_URL", "OPENAI_BASE_URL") ?? "https://generativelanguage.googleapis.com/v1beta"
  ),
  geminiModel: readEnv("GEMINI_MODEL", "OPENAI_MODEL") ?? "gemini-2.5-flash",
  geminiApiFormat:
    process.env.GEMINI_API_FORMAT === "openai" || readEnv("OPENAI_API_KEY", "OPENAI_BASE_URL") ? "openai" : "google",
  geminiOpenAiResponseFormat:
    process.env.GEMINI_OPENAI_RESPONSE_FORMAT === "json_object" || process.env.OPENAI_RESPONSE_FORMAT === "json_object"
      ? "json_object"
      : "none",
...
const recentEvents: JsonObject[] = [];
const activeCalls = new Map<string, ActiveCall>();

let astrbotSocket: WebSocket | null = null;
let astrbotState: AstrBotState = "idle";
let astrbotReconnectTimer: NodeJS.Timeout | null = null;
let astrbotHeartbeatTimer: NodeJS.Timeout | null = null;
...
appServer.on("connection", (socket, request: IncomingMessage) => {
...
function handleAppAuth(client: AppClient, payload: JsonObject): void {
...
function sendClientReady(client: AppClient): void {
  sendJson(client.socket, {
    type: "backend.ready",
    client_id: client.id,
    astrbot_state: astrbotState,
...
function handleAppMessage(client: AppClient, payload: JsonObject): void {
  const type = readString(payload.type, "unknown");
...
function handleAppOutgoingCall(client: AppClient, payload: JsonObject): void {
  ...
  const openingLine =
    readString(payload.opening_line, "我已经接通了。你那边听得到我吗？").trim() ||
    "我已经接通了。你那边听得到我吗？";
...
async function handleAppUserText(client: AppClient, payload: JsonObject, inputMode: string): Promise<void> {
...
async function handleAppUserAudio(client: AppClient, payload: JsonObject): Promise<void> {
  const requestId = readString(payload.request_id, makeId("req"));
  const callId = readString(payload.call_id) || latestActiveCallId();
...
function forwardTextToAstrBot(call: ActiveCall, client: AppClient, requestId: string, text: string, inputMode: string): void {
...
function handleAstrBotPayload(payload: JsonObject): void {
...
function rememberActiveCall(payload: JsonObject): void {
  const callId = readString(payload.call_id);
  const session = (payload.session && typeof payload.session === "object" ? payload.session : {}) as JsonObject;
...
async function writeBackCallSummary(call: ActiveCall, status: string): Promise<void> {
...
async function summarizeCall(call: ActiveCall, status: string): Promise<string> {
...
function buildLocalCallSummary(call: ActiveCall, status: string): string {
...
async function generateAndSendGeminiReply(
  call: ActiveCall,
  client: AppClient,
  requestId: string,
  input: {
...
async function generateGeminiVoiceReply(
  call: ActiveCall,
  input: {
    text?: string;
    audio?: {
...
): Promise<{ reply: string; userNote: string }> {
  const prompt = [
...
async function generateGeminiText(prompt: string, temperature: number): Promise<string> {
  return generateGeminiMultimodal(prompt, undefined, temperature, false);
}

async function generateGeminiMultimodal(
  prompt: string,
  audio: { data: string; mimeType: string; durationMs: number } | undefined,
  temperature: number,
  preferJson: boolean
): Promise<string> {
...
async function generateGoogleGemini(
  prompt: string,
  audio: { data: string; mimeType: string; durationMs: number } | undefined,
  temperature: number,
  preferJson: boolean
  ...
  const response = await fetch(
    `${config.geminiBaseUrl}/${geminiModelPath()}:generateContent?key=${encodeURIComponent(config.geminiApiKey)}`,
    {
      method: "POST",
      headers: {
  ...
  const json = (await response.json().catch(async () => ({ error: { message: await response.text() } }))) as GeminiResponse;
  if (!response.ok) {
...
async function generateOpenAiCompatible(
  ...
  const json = (await response.json().catch(async () => ({ error: { message: await response.text() } }))) as OpenAiChatResponse;
  if (!response.ok) {
    throw new Error(json.error?.message ?? `HTTP ${response.status}`);
  }
...
async function transcribeAudio(audio: { data: string; mimeType: string; durationMs: number }): Promise<string> {
  if (!config.asrApiKey) {
    throw new Error("后端没有配置 ASR_API_KEY / OPENAI_API_KEY，无法把语音转成文字。");
  }

  const text =
    config.asrProvider === "openai_transcriptions"
...
async function transcribeWithOpenAiTranscriptions(audio: {
  ...
  try {
    parsed = JSON.parse(bodyText) as JsonObject;
  } catch {
    parsed = {};
  }
  if (!response.ok) {
    throw new Error(readString(parsed.error && typeof parsed.error === "object" ? (parsed.error as JsonObject).message : "") || bodyText.slice(0, 240) || `HTTP ${response.status}`);
  }
...
function looksLikeModelAnswer(value: string): boolean {
  if (!value) {
    return false;
  }
...
function extractOpenAiText(json: OpenAiChatResponse): string {
...
function parseGeminiReply(rawText: string): { reply: string; userNote: string } {
...
function stringifyAstrBotContext(call: ActiveCall): string {
...
function stringifyVoiceTurns(turns: VoiceTurn[]): string {
  if (!turns.length) {
    return "还没有电话内对话。";
  }
  return turns
...
async function synthesizeReply(payload: JsonObject): Promise<void> {
  const requestId = readString(payload.request_id, makeId("req"));
  const callId = readString(payload.call_id);
  const text = readString(payload.text).trim();
...
async function synthesizeWithMimo(requestId: string, text: string): Promise<string> {
...
async function serveAudio(pathname: string, response: http.ServerResponse): Promise<void> {
...
function sendToAstrBot(payload: JsonObject): boolean {
...
function scheduleAstrBotReconnect(): void {
...
function startAstrBotHeartbeat(): void {
  stopAstrBotHeartbeat();
...
function stopAstrBotHeartbeat(): void {
...
function rememberEvent(event: JsonObject): void {
  recentEvents.push({
    ...event,
...
function broadcastToApps(payload: JsonObject, exceptClientId?: string): void {
...
function parseJsonMessage(data: RawData): JsonObject | null {
  try {
...
function sendHttpJson(response: http.ServerResponse, statusCode: number, payload: JsonObject): void {
...
function safeAudioFileName(value: string, mimeType: string): string {
...
function latestActiveCallId(): string {
  let latest: ActiveCall | null = null;
...
function normalizeVoiceReplyMode(value: string | undefined): VoiceReplyMode {
  return value === "backend_model" ? "backend_model" : "astrbot";
}

function normalizeAsrProvider(value: string | undefined): AsrProvider {
  return value === "openai_transcriptions" ? "openai_transcriptions" : "openai_chat_audio";
}

function normalizeAudioFormat(value: string | undefined): "wav" | "mp3" {
...
function publicBaseUrlFromRequest(request: IncomingMessage): string {
  ...
  const protocol = forwardedProto ?? (((request.socket as unknown as { encrypted?: boolean }).encrypted ? "https" : "http"));
